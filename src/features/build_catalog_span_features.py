...
`seismolib.rust_io.RustData.open(features=...)` accepts, so it drops straight
...
`forecasting/cnn_lstm_catalog_waveform_fusion.py`.
    ...
    python3 src/features/build_catalog_span_features.py \\
...
from seismolib.catalog import AEGEAN_BBOX, days_since_prev_major, load_aegean_events
...
def load_catalog_frame(catalog_path, mc):
    ...
    `seismolib.catalog`'s loaders return arrays rather than a DataFrame with a
